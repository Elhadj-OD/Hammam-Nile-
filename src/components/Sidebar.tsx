...
export const Sidebar: React.FC<SidebarProps> = ({ mobileOpen = false, setMobileOpen }) => {
  ...
  // Prélèvements & Hammam ne concernent que Boutique Homme (Elhadj) et
  // Boutique Femme + Hammam (@hammam) — pas les autres caisses (Esthétique,
  // Coiffure, Épilation, Gym, Buvette).
  const canSeeHammamSections =
    isGerant || currentUser.department === 'boutique_homme' || currentUser.department === 'boutique_femme';
  // Coiffure & Salon et Épilation vendent des prestations, pas des produits
  // physiques — le menu doit dire "Services", pas "Produits".
  const isServiceDept =
    currentUser.department === 'coiffure_salon' || currentUser.department === 'epilation_traditionnelle';
  const produitsLabel = isServiceDept ? 'Services' : 'Produits';
    ...
    <>
      ...
      >
        ...
        <ul className="list-none p-0 m-0 flex flex-col gap-1 flex-1">
          ...
          {/* Prélèvements Hammam - Boutique Homme (Elhadj) + Boutique Femme/Hammam (@hammam) + Admin */}
          ...
          {/* Hammam - Boutique Homme (Elhadj) + Boutique Femme/Hammam (@hammam) + Admin */}
          {canSeeHammamSections && (
            <li
          ...
          {isGerant && (
            ...
            >
              <svg
                ...
                className="w-[17px] h-[17px] shrink-0"
              >
                <path d="M12 2.5l1.9 4.6 4.6 1.9-4.6 1.9-1.9 4.6-1.9-4.6-4.6-1.9 4.6-1.9z" />
