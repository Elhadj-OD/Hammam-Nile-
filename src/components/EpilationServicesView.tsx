...
export const EpilationServicesView: React.FC = () => {
  ...
  const helwaProducts = useMemo(
    () => products.filter(p => p.category === 'epilation_traditionnelle' && p.name.startsWith('Helwa')),
    [products]
  );
  const henneProducts = useMemo(
    () => products.filter(p => p.category === 'epilation_traditionnelle' && p.name.startsWith('Henné')),
    [products]
  );
  const activeProducts = activeTab === 'helwa' ? helwaProducts : henneProducts;

  const mySales = useMemo(() => {
    const now = Date.now();
    return sales.filter(s => {
      if (!currentUser || s.caissier.toLowerCase() !== currentUser.username.toLowerCase()) return false;
      if (!s.items.some(i => i.category === 'epilation_traditionnelle')) return false;
  ...
  }, [sales, currentUser, filterPeriod]);
    ...
    <div className="space-y-6 max-w-[1100px] mx-auto pb-12">
      ...
      <div className="bg-white rounded-2xl p-6 border border-[#E7E0D3] shadow-xs">
        <div className="flex items-center gap-2 text-[#0F4C4A] text-xs font-bold uppercase tracking-wider mb-1">
          ...
          <span>Épilation Traditionnelle</span>
        </div>
        <h1 className="text-2xl font-bold font-display text-[#1C2321]">Helwa & Henné — Services</h1>
        <p className="text-sm text-[#6B7873] mt-1">
          Sélectionnez une prestation ci-dessous, puis encaissez directement — pas de panier boutique, pas de commission.
        </p>
      </div>
      ...
      <div className="flex items-center bg-white p-1 rounded-2xl border border-[#E7E0D3] shadow-xs w-fit">
        <button
          type="button"
          onClick={() => setActiveTab('helwa')}
          className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${
            activeTab === 'helwa' ? 'bg-[#0F4C4A] text-white shadow-xs' : 'text-[#6B7873] hover:text-[#1C2321]'
          }`}
        >
          🪡 Helwa
        </button>
        <button
          type="button"
          onClick={() => setActiveTab('henne')}
          className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${
            activeTab === 'henne' ? 'bg-[#0F4C4A] text-white shadow-xs' : 'text-[#6B7873] hover:text-[#1C2321]'
          }`}
        >
          🎨 Henné
        </button>
