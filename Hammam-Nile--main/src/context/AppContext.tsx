    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          const mariemIndex = ensured.findIndex(u => u.username.toLowerCase() === 'mariem');
          if (mariemIndex >= 0) {
            ensured[mariemIndex] = {
              ...ensured[mariemIndex],
              role: 'caissier',
          ...
          if (sophiaIndex >= 0) {
            ensured[sophiaIndex] = { ...ensured[sophiaIndex], password: '2630', role: 'gerant', name: 'Sophia', gender: 'femme', zone: 'Direction & Gérance' };
          } else {
            ensured.push(INITIAL_USERS_LIST[1]);
          }
          const elhadjIndex = ensured.findIndex(u => u.username.toLowerCase() === 'elhadj');
          if (elhadjIndex >= 0) {
            ensured[elhadjIndex] = { ...ensured[elhadjIndex], password: '3454', name: 'Elhadj', gender: 'homme' };
          } else {
            ensured.push(INITIAL_USERS_LIST[2]);
          }
          return ensured;
        }
      } catch {
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    ...
    if (savedUser) {
      try {
        ...
        if (
          parsed &&
          (parsed.username.toLowerCase() === 'sophia' ||
            parsed.username.toLowerCase() === 'mariem' ||
            parsed.username.toLowerCase() === 'elhadj')
        ) {
          return parsed;
        }
      } catch {
