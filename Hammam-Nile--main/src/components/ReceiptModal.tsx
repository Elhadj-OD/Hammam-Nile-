  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0A3735]/55 backdrop-blur-xs overflow-y-auto">
      ...
      <div className="relative w-full max-w-[370px] max-h-[92vh] overflow-y-auto bg-white rounded-[30px] p-6 shadow-[0_40px_70px_-30px_rgba(10,55,53,0.5)] border border-[#E7E0D3] my-auto text-[#1C2321] animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* --- THERMAL POS TICKET ELEMENT (80mm) --- */}
        <div id="ticket" className="w-full bg-white">
          ...
          <div className="flex flex-col items-center text-center mb-4 pt-1">
            ...
            <div className="text-[11px] text-[#6B7873] mb-2 leading-tight">
              <p className="font-bold text-[#1C2321] text-[12.5px] uppercase tracking-wide">{settings.shopName}</p>
              {settings.slogan && <p className="text-[10px] text-[#6B7873] italic">{settings.slogan}</p>}
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#F7F3EC] text-[11px] font-bold text-[#6B7873] border border-[#E7E0D3]">
              <span>Ticket de Caisse & Reçu</span>
            </div>
          </div>

          {/* Ticket Products Card */}
          <div className="border border-[#E7E0D3] rounded-2xl p-3.5 mb-3.5 bg-white">
            <div className="flex flex-col divide-y divide-[#E7E0D3]">
              {sale.items.map((item, idx) => (
                ...
                >
                  <div className="w-11 h-11 rounded-[10px] bg-[#F7F3EC] flex items-center justify-center text-[20px] shrink-0 overflow-hidden no-print">
                    {item.image ? (
          ...
          {/* Ticket Summary Card */}
          <div className="border border-[#E7E0D3] rounded-2xl p-3.5 mb-3 bg-white">
          ...
          {/* Ticket Footer / Note */}
          <div className="text-center text-[11px] text-[#6B7873] mt-2 pt-2 border-t border-dashed border-[#E7E0D3]">
            <p className="font-medium">{settings.footerNote || 'Merci de votre visite et à très bientôt !'}</p>
          </div>
        </div>

        {/* Ticket Actions (Hidden in Print) */}
        ...
        {/* Confirmation Footer (Hidden in Print) */}
